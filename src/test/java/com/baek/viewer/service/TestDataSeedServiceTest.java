...
class TestDataSeedServiceTest {
    ...
    void buildApiRows_generatesCorrectDistribution() {
        ...
        assertThat(rows.stream().filter(r -> "①-① 차단대상".equals(r.status)).count()).isEqualTo(8);
        assertThat(rows.stream().filter(r -> "①-② 담당자 판단".equals(r.status)).count()).isEqualTo(4);
        assertThat(rows.stream().filter(r -> "①-③ 현업요청 제외대상".equals(r.status)).count()).isEqualTo(2);
        assertThat(rows.stream().filter(r -> "①-④ 사용으로 변경".equals(r.status)).count()).isEqualTo(4);
