...
정적 리소스(`static/**`) 변경이 브라우저/서버 캐시로 즉시 반영되지 않는 경우가 있어,
상단 브랜드 텍스트 **"IT소스 관리포털 — IT Source Management Portal"** 뒤에 UI 버전(`ver1.1.001` 형태)을 표시한다.

- **버전 수정 위치**: `src/main/resources/static/common/nav.js` 의 `APP_UI_VERSION`
- **운영 규칙**: 화면 변경을 배포/반입할 때마다 버전을 올려서 “현재 화면이 최신인지”를 즉시 판별한다.

## 버전업 규칙 (major/minor/patch)

UI 버전 문자열은 `ver<major>.<minor>.<patch>` 형식을 사용한다. (예: `ver1.1.002`)

- **patch 증가**: 버그 수정, 레이아웃/표 컬럼 재배치, 문구 수정, 로깅/표시 개선 등 **기능 동작은 동일하지만 결과/표시가 개선되는 변경**
- **minor 증가**: 화면/데이터 흐름에 새 기능이 추가되거나 사용자가 체감하는 동작이 확장되는 변경 (예: 새 페이지/카드/필터/다운로드 추가)
- **major 증가**: 모듈 단위의 큰 개편, 기존 사용 흐름/데이터 구조의 큰 변경, 호환성에 영향이 큰 변경

원칙: **변경 폭이 애매하면 patch → minor 순으로 보수적으로 올린다.**

## 서버 기동 완료 로그

Windows 등 `run.sh`를 쓰지 못하는 환경에서도 접속 링크를 확인할 수 있도록,
서버 기동 완료 시 콘솔 로그에 **UI 버전 + 주요 페이지 링크 목록**을 출력한다. (`UiVersionStartupLogger`)
...
| `test_suspect_keywords` | 테스트용 의심 키워드 콤마 리스트 (기본 9종: `test,sample,mock,테스트,샘플,demo,dummy,fixture,스텁`). URL 경로/메소드명/컨트롤러명/파일경로/주석/@ApiOperation/@Description, **대소문자 무시**. 영문·숫자만 2~3글자 키워드는 비(문자·숫자) 경계 + 메소드/컨트롤러는 camelCase 토큰 정확 일치; 그 외는 부분일치(contains). `fullUrl` 은 의도적 제외 (도메인 false positive 방지) |
| `snapshot_retention_days` | URL 스냅샷 보관일수. null이면 `repos-config.yml`의 `global.snapshotRetentionDays`(미설정 시 코드 기본값 365) 사용. Extract 완료 후 자동 cleanup 및 수동 정리 버튼에 적용 |

## 스냅샷(Extract 히스토리) — `api_record_snapshot` / `api_record_snapshot_row`

- 목적: “백업(덮어쓰기)”와 달리 **Extract 시점별로 누적 보관**하여 특정 시점 전체 목록 조회 + 두 시점 diff(신규/삭제/변경) 제공
- 생성 트리거: 수동 Extract 완료 시 자동 생성 + `POST /api/snapshots` 수동 생성
- 정리/삭제:
  - 보관주기 정리: `POST /api/snapshots/cleanup`
  - 단건/다건 삭제: `DELETE /api/snapshots/{id}`, `DELETE /api/snapshots?ids=...`, 기간 삭제 `DELETE /api/snapshots/by-date?from&to`

### 메타 테이블: `api_record_snapshot`
- `id` (PK), `snapshot_at`, `snapshot_type`, `label`, `created_ip`, `source_repo`, `record_count`

### 행 테이블: `api_record_snapshot_row`
- 키: `(snapshot_id, repository_name, api_path, http_method)` (UNIQUE)
- 내용: `api_record`의 컬럼 대부분(TEXT 포함) + `snapshot_id`, `source_id(api_record.id)`
...
├─ 4. 상태 판정 기준 (9 leaf v2 — 비IT 요약 + 기술 상세; 자동 6 + 수동 3 + 사용 1)
...
├─ 8. 페이지별 접근 권한
├─ 9. URL 차단 여부 판정 기준
├─ 10. URL 차단여부 플로우차트    (개발자 참고용 — R1: 메서드 본문 UnsupportedOperationException + 메시지 차단)
