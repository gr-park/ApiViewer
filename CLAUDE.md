...
# 상태 (status) — 자동 5가지 + 수동 3가지

| 상태 | 자동 계산 조건 | 비고 |
|------|---------------|------|
| 차단완료 | 메서드 본문 어디서든 `throw new UnsupportedOperationException("...차단...")` — 즉 예외 생성 + 메시지 문자열에 "차단" 포함 — 이면 실질 차단으로 간주(hasUrlBlock=Y). 위치(첫 실행 문장일 필요 없음) 무관. `@Deprecated` 어노테이션 또는 `[URL차단작업]` javadoc 주석 중 하나라도 누락되면 `blockMarkingIncomplete=true` (차단처리미흡) 플래그 동반 — 상태는 동일하게 "차단완료". | 모든 수정 불가. 차단처리미흡 플래그는 주석/어노테이션 보완 후 재추출 시 자동 해제. |
| 최우선 차단대상 | 호출 0건 + 커밋 1년 경과 | 자동 |
| 후순위 차단대상 | 수동 설정 (침해사고 로그, IT담당자 검토건) | 수동 |
| 추가검토필요 차단대상 | 호출 0건+커밋 1년 미만 / 호출 1~N건+커밋 1년 경과 | 자동 |
| 사용 | 기본값 | — |

## 수동 판단 상태 (담당자 결정, 자동 재계산 제외)

`ApiStorageService.MANUAL_STATUSES` 로 정의된 3개 상태값. 사용자가 일괄변경/단건수정으로 이 상태를 선택하면 `statusOverridden=true` 가 자동 ON 되어 추출 재계산 시 보존된다. `calculateStatus` 도 현재 상태가 MANUAL_STATUSES 인 레코드는 그대로 반환(이중 보호).

| 상태 | 의미 | 입력 경로 |
|------|------|----------|
| 최우선 차단대상 → 사용 | 자동 판정으론 최우선 차단대상이지만 담당자 검토 결과 사용중으로 확정 | 일괄변경 / 단건수정 / 엑셀 업로드 |
| 후순위 차단대상 → 사용 | 자동 판정으론 후순위 차단대상이지만 담당자 검토 결과 사용중으로 확정 | 동일 |
| 현업요청 사용 | 현업의 명시적 사용 요청에 의해 차단 대상에서 제외 (협조전·메일 근거 보존 권장) | 동일 |
...
| 컬럼 | 허용 값 |
|------|---------|
| 상태 | 사용, 차단완료, 최우선 차단대상, 후순위 차단대상, 추가검토필요 차단대상, 최우선 차단대상 → 사용, 후순위 차단대상 → 사용, 현업요청 사용 |
| 상태확정 | 확정, 미확정 |
...
├─ 4. 상태 판정 기준 (자동 5가지 + 수동 3가지) ※ 자동: 차단완료/최우선/후순위/추가검토필요 차단대상/사용. 수동: 최우선 차단대상 → 사용 / 후순위 차단대상 → 사용 / 현업요청 사용 (담당자 결정, 자동 재계산 제외)
