...
# 상태 (status) — 13 leaf, 상위 3-tier (사용 / ① 차단대상 / ② 추가검토대상)

DB `status` 컬럼은 leaf 값을 직접 저장한다. 화면 라벨 = DB 값 (전체 표기).
상위 카테고리 ① 차단대상 / ② 추가검토대상 은 **계산 결과**로만 분류되며 직접 수정할 수 없다.
...
|------|-------------|---------------|------|
| — | `사용` | 기본값 (그 외) | 자동 |
| ① 차단대상 | `①-① 차단완료` | `hasUrlBlock=Y` | 자동 (수정 불가) |
| ① 차단대상 | `①-② 호출0건+변경없음` | 호출 0 + 모든 커밋 1년 경과 | 자동 |
| ① 차단대상 | `①-③ 호출0건+변경있음(로그)` | 호출 0 + 로그작업 커밋 제외 시에만 1년 경과 | 자동 |
| ① 차단대상 | `①-④ 업무종료` | (옛 후순위 차단대상) — 수동 지정 | 수동 (leaf) |
| ① 차단대상 | `①-⑤ 현업요청 차단제외` | `reviewResult='차단대상 제외'` | 자동 |
| ① 차단대상 | `차단대상 → 사용` | 차단대상에서 담당자 판단 → 사용 (수동) | 수동 (옵션A: 라벨 그대로) |
| ② 추가검토대상 | `②-① 호출0건+로그건` | 호출 0 + 1년 미만 + 모든 커밋 "로그"/"불필요" 키워드 | 자동 |
| ② 추가검토대상 | `②-② 호출0건+변경있음` | 호출 0 + 1년 미만 + 비-로그성 커밋 1건↑ | 자동 |
| ② 추가검토대상 | `②-③ 호출 1~reviewThreshold건` | 호출 1~threshold + 1년 경과 | 자동 |
| ② 추가검토대상 | `②-④ 호출 reviewThreshold+1건↑` | 호출 threshold+1~upper + 1년 경과 | 자동 |
| ② 추가검토대상 | `검토필요 → 사용` | 추가검토대상에서 담당자 판단 → 사용 (수동) | 수동 (옵션A: 라벨 그대로) |

상위 카테고리 매핑: status가 `①-` 으로 시작 → ① 차단대상, `②-` 시작 → ② 추가검토대상,
`차단대상 → 사용` → ①-⑥ 컬럼, `검토필요 → 사용` → ②-⑤ 컬럼 (옵션A).
...
| 컬럼 | 의미 | 갱신 시점 |
|------|------|----------|
| `log_work_excluded` | ①-② vs ①-③ 분기 보조값 | umbrella 외부 → 차단 first-time 진입 시 |
| `recent_log_only`   | ②-① vs ②-② 분기 보조값 | umbrella 외부 → 추가검토 first-time 진입 시 |
...
| 키 | 기본값 | 의미 |
|----|--------|------|
| `reviewThreshold` | 3 | ②-③ "호출 1~N건" 의 N. ②-③/②-④ 분기 기준 |
| `reviewUpperThreshold` | 10 | 추가검토대상 호출 상한. 이 값 초과면 `사용` |

## 수동 판단 상태 (담당자 결정, 옵션A 라벨 유지)

`ApiStorageService.MANUAL_STATUSES` 로 정의된 2개 상태값. 사용자가 일괄변경/단건수정으로 이 상태를 선택하면 `statusOverridden=true` 가 자동 ON 되어 추출 재계산 시 보존된다. `calculateStatus` 도 현재 상태가 MANUAL_STATUSES 인 레코드는 그대로 반환(이중 보호). 대시보드 "차단대상 진행사항" 표에서는 ①-⑥ / ②-⑤ 컬럼으로 자동 분류되어 노출.

| 상태 | 의미 | 입력 경로 |
|------|------|----------|
| `차단대상 → 사용` | 자동 판정으론 ①-②/①-③ 등 차단대상이지만 담당자 검토 결과 사용중으로 확정 | 일괄변경 / 단건수정 / 엑셀 업로드 |
| `검토필요 → 사용` | 자동 판정으론 ②-* 추가검토대상이지만 담당자 검토 결과 사용중으로 확정 | 동일 |
