...
- IT 비전문가용 단순 화면: 레포지토리, 업무명, 팀, 담당자, API경로, URL호출, 내용, 호출건수, 비고, 차단대상, 차단대상기준
- 인라인 편집: 현업검토결과(select), 현업검토의견(text), 현업 팀(text), 현업 담당자(text)
- 현업검토일시: 검토 필드 업데이트 시 서버에서 자동 갱신 (사용자 수정 불가)
- 체크박스 + 일괄 변경 바: 여러 건 선택하여 검토결과/의견/현업팀/담당자 일괄 변경
- 통계: 전체, 최우선, 후순위, 검토완료, 미검토
- 엑셀 다운로드 지원

### 5-4. 조회화면 팀/담당자 인라인 편집
- viewer.html에서 팀, 담당자 셀 클릭 → 인라인 편집 → `teamOverride`/`managerOverride` 필드에 저장
- 표시 우선순위: `teamOverride || repoConfig.teamName` (개별 레코드 오버라이드 > 레포 설정)
- viewer.html에 현업검토결과 컬럼 추가 (읽기 전용 배지)
