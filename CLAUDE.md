...
# 상태 (status) — 13 leaf, 상위 3-tier (사용 / (1) 차단대상 / (2) 추가검토대상)

DB `status` 컬럼은 leaf 값을 직접 저장한다. 화면 라벨 = DB 값 (전체 표기).
상위 카테고리 (1) 차단대상 / (2) 추가검토대상 은 **계산 결과**로만 분류되며 직접 수정할 수 없다.
...
|------|-------------|---------------|------|
| — | `사용` | 기본값 (그 외) | 자동 |
| (1) 차단대상 | `(1)-(1) 차단완료` | `hasUrlBlock=Y` | 자동 (수정 불가) |
| (1) 차단대상 | `(1)-(2) 호출0건+변경없음` | 호출 0 + 모든 커밋 1년 경과 | 자동 |
| (1) 차단대상 | `(1)-(3) 호출0건+변경있음(로그)` | 호출 0 + 로그작업 커밋 제외 시에만 1년 경과 | 자동 |
| (1) 차단대상 | `(1)-(4) 업무종료` | (옛 후순위 차단대상) — 수동 지정 | 수동 (leaf) |
| (1) 차단대상 | `(1)-(5) 현업요청 차단제외` | `reviewResult='차단대상 제외'` | 자동 |
| (1) 차단대상 | `차단대상 → 사용` | 차단대상에서 담당자 판단 → 사용 (수동) | 수동 (옵션A: 라벨 그대로) |
| (2) 추가검토대상 | `(2)-(1) 호출0건+로그건` | 호출 0 + 1년 미만 + 모든 커밋 "로그"/"불필요" 키워드 | 자동 |
| (2) 추가검토대상 | `(2)-(2) 호출0건+변경있음` | 호출 0 + 1년 미만 + 비-로그성 커밋 1건↑ | 자동 |
| (2) 추가검토대상 | `(2)-(3) 호출 1~reviewThreshold건` | 호출 1~threshold + 1년 경과 | 자동 |
| (2) 추가검토대상 | `(2)-(4) 호출 reviewThreshold+1건↑` | 호출 threshold+1~upper + 1년 경과 | 자동 |
| (2) 추가검토대상 | `검토필요 → 사용` | 추가검토대상에서 담당자 판단 → 사용 (수동) | 수동 (옵션A: 라벨 그대로) |

상위 카테고리 매핑: status가 `(1)-` 으로 시작 → (1) 차단대상, `(2)-` 시작 → (2) 추가검토대상,
`차단대상 → 사용` → (1)-(6) 컬럼, `검토필요 → 사용` → (2)-(5) 컬럼 (옵션A).

## 자동 재계산 규칙 (umbrella sticky)

`ApiStorageService.calculateStatus` 는 `statusOverridden=false` 인 레코드에 대해서만 동작:

1. **MANUAL_STATUSES 보존** — `차단대상 → 사용` / `검토필요 → 사용` 은 항상 그대로
2. **`hasUrlBlock=Y` → `(1)-(1) 차단완료`** (강한 신호, umbrella 무시)
3. **`reviewResult='차단대상 제외'` → `(1)-(5) 현업요청 차단제외`** (강한 신호)
4. **현재 (1)-* 또는 (2)-* leaf** (umbrella 내부) → 조건이 `사용`을 가리킬 때만 `사용` 으로 전이.
   그 외에는 leaf 보존 (차단↔추가검토 전이 금지, umbrella 내 leaf 변경 금지)
5. **현재 `사용` 또는 `(1)-(1) 차단완료`** (umbrella 외부) → 조건에 따라 leaf 자유 할당

이 sticky 규칙으로 인해 한 번 분류된 차단대상/추가검토대상 leaf 는 호출수·커밋·플래그가 변해도
그대로 유지되며, 사용자가 수동으로 `사용` 으로 되돌리거나 조건이 명백히 `사용` 을 가리킬 때만 전이된다.

## 보조 플래그 (자동 분기 시 보조 정보)

| 컬럼 | 의미 | 갱신 시점 |
|------|------|----------|
| `log_work_excluded` | (1)-(2) vs (1)-(3) 분기 보조값 | umbrella 외부 → 차단 first-time 진입 시 |
| `recent_log_only`   | (2)-(1) vs (2)-(2) 분기 보조값 | umbrella 외부 → 추가검토 first-time 진입 시 |

sticky 규칙으로 umbrella 내부에 있는 동안에는 보조 플래그도 갱신되지 않는다.

## 임계값 설정 (GlobalConfig)

| 키 | 기본값 | 의미 |
|----|--------|------|
| `reviewThreshold` | 3 | (2)-(3) "호출 1~N건" 의 N. (2)-(3)/(2)-(4) 분기 기준 |
| `reviewUpperThreshold` | 10 | 추가검토대상 호출 상한. 이 값 초과면 `사용` |

## 수동 판단 상태 (담당자 결정, 옵션A 라벨 유지)

`ApiStorageService.MANUAL_STATUSES` 로 정의된 2개 상태값. 사용자가 일괄변경/단건수정으로 이 상태를 선택하면 `statusOverridden=true` 가 자동 ON 되어 추출 재계산 시 보존된다. `calculateStatus` 도 현재 상태가 MANUAL_STATUSES 인 레코드는 그대로 반환(이중 보호). 대시보드 "차단대상 진행사항" 표에서는 (1)-(6) / (2)-(5) 컬럼으로 자동 분류되어 노출.

| 상태 | 의미 | 입력 경로 |
|------|------|----------|
| `차단대상 → 사용` | 자동 판정으론 (1)-(2)/(1)-(3) 등 차단대상이지만 담당자 검토 결과 사용중으로 확정 | 일괄변경 / 단건수정 / 엑셀 업로드 |
| `검토필요 → 사용` | 자동 판정으론 (2)-* 추가검토대상이지만 담당자 검토 결과 사용중으로 확정 | 동일 |

| 상태확정 | 설명 |
|----------|------|
| 확정 | `statusOverridden=true`, 자동 재계산 안 함, 수정 불가 (파란 배지) |
| 미확정 | `statusOverridden=false`, 자동 재계산 대상 (회색 배지) |
| 변경불가 | `(1)-(1) 차단완료` 건, 일체 수정 불가 (비활성 배지) |
...
| 컬럼 | 허용 값 |
|------|---------|
| 상태 | 사용, 차단완료, 최우선 차단대상, 후순위 차단대상, 검토필요대상, 차단대상 → 사용, 검토필요 → 사용 |
| 상태확정 | 확정, 미확정 |
...
├─ 4. 상태 판정 기준 (자동 5가지 + 수동 2가지) ※ 자동: 차단완료/최우선 차단대상/후순위 차단대상/검토필요대상/사용. 수동: 차단대상 → 사용 / 검토필요 → 사용 (담당자 결정, 자동 재계산 제외)
...
└─ 12. 테이블 / 컬럼 명세서        (block_marking_incomplete 컬럼 포함)
```
